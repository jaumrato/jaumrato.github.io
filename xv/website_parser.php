...
class WebsiteParser
{
	...
	private $videoos_expression = "/((\"|')(http|www).+?(\.mp4\?(.*?)\w+|\.mp4)('|\"))/i";
    ...
    public function getMetaTags($grab = false)
    {
	 ...
	 public function getvideoo($grab = false)
    {
        $videoos = array();
        ...
        if (!is_null($this->content)) {
            ...
            preg_match_all($this->videoos_expression, $this->content, $match_videoos);

            if (isset($match_videoos[2]) && count($match_videoos[2])) {
                foreach ($match_videoos[2] as $key => $match_videoo) {

                    $key = trim($match_videoos[1][$key]);
                    $match_videoo = trim($match_videoo);

                    if ($match_videoo) {
                        $videoos[] = array($key, $match_videoo);
                    }
                }
            }
        }
